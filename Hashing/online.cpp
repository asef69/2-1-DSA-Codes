...
public:
    ...
    void printProbeSequence(const string &key) const
    {
        vector<int> sequence;
        int i = 0;
        ...
        while (i < tableSize)
        {
            ...
            // Stop if we find an EMPTY slot (end of probe sequence)
            if (table[index].state == EMPTY)
            {
                break;
            }
...
void testProbeSequence()
{
    cout << "Generating 10,000 unique words of length 10..." << endl;
    RandomWordGenerator generator(42);
    vector<string> words = generator.generateUniqueWords(10000, 10);
    
    // Create hash tables for all probing methods
    LinearProbingTable linearHT("hash1");
    QuadraticProbingTable quadraticHT("hash1");
    DoubleHashingTable doubleHT("hash1");
    CustomProbingTable customHT("hash1");
    
    // Insert all 10000 words into all hash tables
    for (size_t i = 0; i < words.size(); i++)
    {
        linearHT.insert(words[i], i + 1);
        quadraticHT.insert(words[i], i + 1);
        doubleHT.insert(words[i], i + 1);
        customHT.insert(words[i], i + 1);
    }

    // Take input for n keys
    int n;
    cin >> n;
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        // Print probe sequences for all methods
        cout << "Linear Probing: ";
        linearHT.printProbeSequence(key);
        
        cout << "Quadratic Probing: ";
        quadraticHT.printProbeSequence(key);
        
        cout << "Double Hashing: ";
        doubleHT.printProbeSequence(key);
        
        cout << "Custom Probing: ";
        customHT.printProbeSequence(key);
        
        if (i < n - 1) {
            cout << endl;  // Add blank line between different keys
        }
    }
}
...
int main()
{
    // Uncomment the line below to run the original tests
    // BasicFunctionality();
    // cout << endl;
    // runPerformanceEvaluation();
    
    // Run the probe sequence test
    testProbeSequence();
