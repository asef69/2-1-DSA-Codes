...
private:
    ...
    int findGroupForInsert(const string &groupId)
    {
        int base = hash1(groupId);
        int firstDeleted = -1;
        for (int i = 0; i < tableSize; i++)
...
public:
    PermissionDB(int n) : tableSize(n), outerTable(n) {}
    void insert(const string &groupId, const string &userId, const string &permission)
    {
        int groupIndex = findGroupForInsert(groupId);
        if (groupIndex == -1)
        {
            return;
        }
        if (outerTable[groupIndex].state != ACTIVE)
        {
            ...
            outerTable[groupIndex].useOrder.clear();
        }
        ...
        outerTable[groupIndex].useOrder.push_back(userId);
    }
    void searchUser(const string &groupId, const string &userId)
    {
        ...
        for (int i = 0; i < tableSize; i++)
        {
            ...
            {
                cout << inner[index].permission << endl;
                return;
            }
        }
        cout << "User not Found " << groupId << endl;
    }
    void searchGroup(const string &groupId)
    {
        int groupIndex = findGroupIndex(groupId);
        if (groupIndex == -1)
        {
        ...
        GroupEntry &group = outerTable[groupIndex];
        bool firstPrinted = true;
        for (const auto &userId : group.useOrder)
        {
            int base = hash2(userId);
            for (int i = 0; i < tableSize; i++)
            {
    ...
    void removeUser(const string &groupId, const string &userId)
    {
        ...
        for (int i = 0; i < tableSize; i++)
        {
            ...
            if (inner[index].state == ACTIVE && inner[index].userId == userId)
            {
                cout << "(" << inner[index].userId << "," << inner[index].permission << ") deleted" << endl;
                inner[index].state = DELETED;
                vector<string> &order = outerTable[groupIndex].useOrder;
